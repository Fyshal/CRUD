...
namespace CRUD
{
    public partial class SignUP : Form
    {
        ...
        {
            SqlConnection con = new SqlConnection("Data Source=DESKTOP-2GMFMFD\\SQLEXPRESS;Initial Catalog=CRUD;Integrated Security=True;Encrypt=True;TrustServerCertificate=True");
            con.Open();
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection con = new SqlConnection("Data Source=DESKTOP-2GMFMFD\\SQLEXPRESS;Initial Catalog=CRUD;Integrated Security=True;Encrypt=True;TrustServerCertificate=True"))
                {
                    con.Open();
                    string updateQuery = "UPDATE crud SET email=@email, name=@name, username=@username, password=@password WHERE id=@id";
                    using (SqlCommand cmd = new SqlCommand(updateQuery, con))
                    {
                        // Add parameters with explicit conversion
                        cmd.Parameters.AddWithValue("@email", txtEmail.Text);
                        cmd.Parameters.AddWithValue("@name", txtName.Text);
                        cmd.Parameters.AddWithValue("@username", txtUser.Text);
                        cmd.Parameters.AddWithValue("@password", txtpass.Text);
